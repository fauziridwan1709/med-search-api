...
class VBEPostings:
    ...
    def encode(postings_list):
        ...
        bytes
            bytearray yang merepresentasikan urutan integer di postings_list
        """
        # list of posting bisa diganti jadi list of gaps 
        # kecuali nilai dari posting yang pertama
        gap_postings_list = postings_list[:1]

        for i in range(1, len(postings_list)):
            gap = postings_list[i] - postings_list[i - 1]
            gap_postings_list.append(gap)

        # encode gap posting list menjadi bytearray
        bytes_result = VBEPostings.vb_encode(gap_postings_list) 
        return bytes_result
    ...
    def vb_decode(encoded_bytestream):
        ...
        for byte in encoded_bytestream:
            # Apabila belum menemukan penanda untuk byte terakhir.
            if byte < 128: 
                n = 128 * n + byte
            
            # Menemukan penanda byte terakhir.
            # append ke dalam list reset n.
            else:
                n = 128 * n + (byte - 128)
                numbers.append(n)
                n = 0
    ...
    def decode(encoded_postings_list):
        ...
        List[int]
            list of docIDs yang merupakan hasil decoding dari encoded_postings_list
        """
        # decode encoded postings list agar menjadi list of numbers.
        # dimana list of numbers itu sendiri merupakan list of gap.
        gap_postings_list = VBEPostings.vb_decode(encoded_postings_list)
        postings_list = gap_postings_list[:1]

        for i in range(1, len(gap_postings_list)):
            gap_postings_list[i] += gap_postings_list[i-1]
            postings_list.append(gap_postings_list[i])
