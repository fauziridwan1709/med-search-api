...
def dcg(ranking):
  """ menghitung search effectiveness metric score dengan 
      ...
      Float
        score DCG
  """
  # Berdasarkan formula dari discounted cumulative gain
  score_dcg = 0.0
  for i in range(len(ranking)):
    score_dcg += ranking[i] / math.log2(i+2)
  return score_dcg
...
def ap(ranking):
  ...
  """
  score_ap = 0.0
  #  banyaknya dokumen relevan di koleksi
  R = sum(ranking)
  for i in range(len(ranking)):
      score_ap += (sum(ranking[:i+1]) / len(ranking[:i+1]) / R) * ranking[i]
  return score_ap
