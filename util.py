...
def sorted_merge_posts_and_tfs(posts_tfs1, posts_tfs2):
    ...
    pointer2 = 0
    sorted_list = []
    while pointer1 < len(posts_tfs1) or pointer2 < len(posts_tfs2):
        if pointer1 >= len(posts_tfs1):
            sorted_list.append(posts_tfs2[pointer2])
            pointer2 += 1
            continue
        elif pointer2 >= len(posts_tfs2):
            sorted_list.append(posts_tfs1[pointer1])
            pointer1 += 1
            continue
        
        doc_id1, tf1 = posts_tfs1[pointer1]
        doc_id2, tf2 = posts_tfs2[pointer2]
        if doc_id1 == doc_id2:
            sorted_list.append((doc_id1, tf1 + tf2))
            pointer1 += 1
            pointer2 += 1
        elif doc_id1 < doc_id2:
            sorted_list.append((doc_id1, tf1))
            pointer1 += 1
        else:
            sorted_list.append((doc_id2, tf2))
            pointer2 += 1
    return sorted_list
